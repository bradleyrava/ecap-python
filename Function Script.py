...
def eta_min_fcn(lambda_param, p_flip, pt, omega, basis_0, basis_1, basis_sum, basis_0_grid, basis_1_grid):
    ...
    ## Constraint vectors
    Amat = np.array(basis_0_grid[end_row])
    b_vec = np.array([0.0])
...
def tweedie_est(lambda_param, gamma_param, theta_param, p_old_new, p_old_new_flip, pt,
    ...
    sigma2_hat = gamma_param * p_old_new_flip * (1 - p_old_new_flip) + \
                 (gamma_param ** 2) * p_old_new_flip * (1 - p_old_new_flip) * (g_hat_d1 - 2)

    exp_p_hat = mu_hat + 0.5 * theta_param * \
                (-mu_hat - 6 * mu_hat * sigma2_hat - 2 * mu_hat ** 3 + 3 * sigma2_hat + 3 * mu_hat ** 2)
    var_p_hat = ((1 - 0.5 * theta_param) ** 2) * sigma2_hat + theta_param * sigma2_hat * \
                (9 * (mu_hat ** 4) * theta_param - 18 * (mu_hat ** 3) * theta_param + 9 * (mu_hat ** 2) * theta_param -
