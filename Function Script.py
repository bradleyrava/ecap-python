...
def eta_min_fcn(lambda_param, p_flip, pt, omega, basis_0, basis_1, basis_sum, basis_0_grid, basis_1_grid):
    n = basis_1.shape[0]
    n_grid = basis_1_grid.shape[0]
    end_row = np.where(pt==0.5)

    ## Set up into the correct form
    Dmat = (2*((1/n) * basis_sum + lambda_param*omega))
    Dmat = Dmat.astype(np.double)
    dvec_terms = ((1-2*p_flip)*basis_0.transpose()).transpose() + ((p_flip*(1-p_flip))*basis_1.transpose()).transpose()
    dvec = (2/n)*dvec_terms.sum(axis=0)

    ## Constraint vectors
    b_vec1 = np.array([0.0])
    bvec = b_vec1

    Amat_part1 = basis_0_grid.iloc[end_row]
    Amat = Amat_part1

    ## Convert to array
    Dmat = matrix(Dmat.to_numpy())
    dvec = matrix(dvec.to_numpy())
    bvec = matrix(np.array([bvec]))
    Amat = matrix(Amat.to_numpy())

    try:
        return_obj = np.array(solvers.qp(P=Dmat, q=dvec, A=Amat, b=bvec)['x'])
    except:
        return_obj = np.linspace(0, 1000, 46)
    return return_obj


def risk_hat_fcn(eta, b_g_test, b_g_d1_test, n, p_test):
    eta = pd.DataFrame(eta)
    g_hat = np.concatenate(np.dot(b_g_test, eta))
    g_hat_d1 = np.concatenate(np.dot(b_g_d1_test, eta))
...
def risk_cvsplit_fcn(lambda_grid, train_index, test_index, basis_0, basis_1, probs_flip, pt, omega, basis_0_grid, basis_1_grid):
    b_g_cv = basis_0.iloc[train_index]
    b_g_d1_cv = basis_1.iloc[train_index]
    p_train = probs_flip.iloc[train_index]
    ...
    sum_b_d1_trian = b_g_d1_cv.transpose().dot(np.repeat(1, b_g_d1_cv.shape[0]))

    ## Do this inversion for every value of lambda
    eta_g = [eta_min_fcn(l, p_train, pt, omega, b_g_cv, b_g_d1_cv, basis_sum_train, basis_0_grid, basis_1_grid)
             for l in lambda_grid]

    ## Now, lets consider the test data
    b_g_test = basis_0.iloc[test_index]
    b_g_d1_test = basis_1.iloc[test_index]
    p_test = probs_flip.iloc[test_index]
    n = b_g_test.shape[0]
    basis_sum = b_g_test.transpose().dot(b_g_test)
...
def tweed_adj_fcn(lambda_param, gamma_param, theta_param, p_tilde, p_flip, probs, omega, basis_0, basis_1, basis_sum,
                  basis_0_grid, basis_1_grid, win_index, lose_index):
    eta_hat = eta_min_fcn(lambda_param, p_tilde, pt, omega, basis_0, basis_1, basis_sum, basis_0_grid, basis_1_grid)
...
def tweedie_est(lambda_param, gamma_param, theta_param, p_old_new, c, pt,
    ...
    exp_p_hat = mu_hat + 0.5 * theta_param * (-mu_hat - 6 * mu_hat * sigma2_hat - 2 * mu_hat ** 3 + 3 * sigma2_hat + 3 * mu_hat ** 2)
    var_p_hat = ((1 - 0.5 * theta_param) ** 2) * sigma2_hat + theta_param * sigma2_hat * (9 * (mu_hat ** 4) * theta_param - 18 * (mu_hat ** 3) * theta_param + 9 * (mu_hat ** 2) * theta_param -
                                                                                          (1 - theta_param * (1 / 2)) * (3 * (mu_hat ** 2) - 3 * mu_hat))
